...
function init (client, d3) {
  ...
  renderer.drawUams = function drawUams(client) {
    _.forEach(client.ddata.uams, function eachUam (d) {
      renderer.drawUam(d);
    });
  };

  renderer.drawUam = function drawUam(uam) {
    function uamTooltip() {
      client.tooltip.transition().duration(TOOLTIP_TRANS_MS).style('opacity', .9);
      client.tooltip
        .html(
          '<strong>' + translate('Time') + ':</strong> ' + client.formatTime(new Date(uam.created_at)) + '<br/>' +
          '<strong>Algorithm:</strong> ' + uam.algorithm + '<br/>' +
          '<strong>Carbs:</strong> ' + uam.carbs + '<br/>'
        )
        .style('left', tooltipLeft())
        .style('top', (d3.event.pageY + 15) + 'px');
    }
    ...
    var innerWidth = window && window.innerWidth || -1;
    // don't render the treatment if it's not visible
    if (Math.abs(chart().xScale(new Date(uam.created_at))) > innerWidth) {
      return;
    }

    var uamDots = chart().focus.selectAll('treatment-insulincarbs')
      .data([
        {
          'element': '',
          'color': 'cyan',
          'inner': 3 * Math.log(uam.carbs) + 5,
          'outer': 3 * Math.log(uam.carbs) + 7
        }
      ])
      .enter()
      .append('g')
      .attr('class', 'draggable-treatment')
      .attr('transform', 'translate(' + chart().xScale(new Date(uam.created_at)) + ', ' + chart().yScale(Number(40)) + ')')
      .on('mouseover', uamTooltip)
      .on('mouseout', hideTooltip);
    ...
    var label = uamDots.append('g')
      .attr('class', 'path')
      .attr('id', 'label')
      .style('fill', 'white');
    label.append('text')
      .style('font-size', 40 / renderer.bubbleScale())
      .style('text-shadow', '0px 0px 10px rgba(0, 0, 0, 1)')
      .attr('text-anchor', 'middle')
      .attr('dy', function(d) {
        return -d.inner;
