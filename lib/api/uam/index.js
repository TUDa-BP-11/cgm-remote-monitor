...
function configure(app, wares, ctx) {
    ...
    api.use(ctx.authorization.isPermitted('api:uam:read'));

    // List activity data available
    api.get('/uam', function(req, res) {
        var ifModifiedSince = req.get('If-Modified-Since');
        ctx.uam.list(req.query, function(err, results) {
            var d1 = null;
    ...
    function config_authed(app, api, wares, ctx) {
        ...
        function post_response(req, res) {
            ...
            ctx.uam.create(activity, function(err, created) {
                if (err) {
        ...
        api.post('/uam/', wares.bodyParser({
            limit: 1048576 * 50
        }), ctx.authorization.isPermitted('api:uam:create'), post_response);

        api.delete('/uam/:_id', ctx.authorization.isPermitted('api:uam:delete'), function(req, res) {
            ctx.uam.remove(req.params._id, function() {
                res.json({});
            });
        });

        // update record
        api.put('/uam/', ctx.authorization.isPermitted('api:uam:update'), function(req, res) {
            var data = req.body;
