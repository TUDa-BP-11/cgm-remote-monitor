...
function loadUams(ddata, ctx, callback) {
    var dateRange = {
        $gte: new Date(ddata.lastUpdated - (ONE_DAY * 8)).toISOString()
    };
    if (ddata.page && ddata.page.frame) {
    ...
    console.log('searching uams q', tq);
    ctx.uam.list(tq, function(err, results) {
        if (!err && results)
            mergeToUams(ddata, results);
        callback();
    })
}

function mergeToUams(ddata, results) {
    var filtered = _.filter(results, function hasId(uam) {
        return !_.isEmpty(uam._id);
    });

    var uams = _.map(filtered, function update(uam) {
        uam.mills = new Date(uam.created_at).getTime();
        return uam;
    });

    ddata.uams = _.unionWith(ddata.uams, uams, function(a, b) {
        return a._id.toString() == b._id.toString();
    });
}
